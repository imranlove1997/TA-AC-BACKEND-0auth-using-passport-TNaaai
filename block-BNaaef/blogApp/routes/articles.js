...
router.get('/:slug', (req, res) => {
    ...
    Article.findOne({ slug }).populate('author', 'username').populate({ path: 'comments', populate: { path: 'author', select: 'username' } }).exec((err, article) => {
        if(err) return next(err);
...
router.get('/:slug/edit', (req, res) => {
    var slug = req.params.slug;
    Article.findOne({ slug }).populate('author', 'username').exec((err, article) => {
        if(err) return next(err);
...
router.post('/:slug/comment', (req, res, next) => {
    ...
    Comment.create(req.body, (err, comment) => {
        if(err) return next(err);
        Article.findOneAndUpdate({ slug: slug }, {$push: { comments: comment.id}}, (err, articleUpdated) => {
            if(err) return next(err);
...
router.get('/:slug/:method', (req, res, next) => {
    ...
    if(method === 'like') {
        Article.findOneAndUpdate({ slug: slug }, { $inc: { likes: 1 } }, (err, liked) => {
            if(err) return next(err);
            return res.redirect('/articles/' + liked.slug);
        })
    }
    if(method === 'dislike') {
        Article.findOneAndUpdate({ slug: slug }, { $inc: { likes: -1 }}, (err, disliked) => {
            if(err) return next(err);
